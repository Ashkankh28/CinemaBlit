...
if (isset($_POST['namefamily']) && !empty($_POST['namefamily']) &&
    ...
    if (isset($_POST['pass']) && !empty($_POST['pass'])) {
        ...
        $newPass = password_hash($rawPass, PASSWORD_DEFAULT);
    }
    ...
    if ($newPass !== null) {
        ...
        mysqli_stmt_bind_param($stmt, "sssssi", $namefamily, $username, $newPass, $email, $phone, $id);
    } else {

        $stmt = mysqli_prepare($link, "UPDATE users SET namefamily = ?, username = ?, email = ?, phone = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt, "ssssi", $namefamily, $username, $email, $phone, $id);
    }
